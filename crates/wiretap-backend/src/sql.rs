...
/// since `can_frame` became `capture_frame`, and every deployed WireTAP desktop
/// labels whatever this gateway returns as `can` without asking — the string is
/// hardcoded in its `apiclient.rs` and `backend_api.rs`. A query that widened by
...
pub async fn byte_changes(
    ...
    let query = format!(
        "WITH ordered_frames AS (\
            ...
            FROM public.capture_frame {filter_args} ORDER BY ts) \
...
pub async fn frame_changes(
    ...
    let query = format!(
        "WITH ordered_frames AS (\
            ...
            FROM public.capture_frame {w} ORDER BY ts) \
...
pub async fn mux_statistics(
    ...
    let w = p.filter.where_clause(&mut args);
    let src = format!(
        "SELECT data_bytes FROM public.capture_frame {w} \
...
pub async fn first_last(
    ...
    let w = p.filter.where_clause(&mut args);
    let select =
        "SELECT (EXTRACT(EPOCH FROM ts) * 1000000)::float8 AS timestamp_us, data_bytes FROM public.capture_frame";
    ...
    let query = format!(
        "SELECT (trunc((EXTRACT(EPOCH FROM ts) * 1000000) / {bucket_us}) * {bucket_us})::float8 AS bucket_start_us, \
         ...
         FROM (SELECT ts, EXTRACT(EPOCH FROM ts - LAG(ts) OVER (ORDER BY ts)) * 1000000 AS dt_us \
               FROM (SELECT ts FROM public.capture_frame {w} ORDER BY ts LIMIT {limit}) f) s \
...
pub async fn distribution(
    ...
    let query = format!(
        "SELECT public.get_byte_safe(data_bytes, ${byte_idx}::int4) AS value, COUNT(*) AS count \
         FROM public.capture_frame {w} GROUP BY value ORDER BY count DESC"
    );
    let rows = client
        .query(&query, &args.refs())
        .await
    ...
    let query = format!(
        "SELECT (EXTRACT(EPOCH FROM prev_ts) * 1000000)::float8 AS gap_start_us, \
         ...
         FROM (SELECT ts, LAG(ts) OVER (ORDER BY ts) AS prev_ts \
               FROM (SELECT ts FROM public.capture_frame {w}) f) s \
...
async fn rollup_available(client: &Client) -> bool {
    client
        .query_one(
            "SELECT to_regclass('public.capture_frame_hourly') IS NOT NULL",
            &[],
        )
        .await
            ...
            .query(
                &format!(
                    "SELECT id, extended, sum(frame_count)::int8 AS cnt, \
                     ...
                     FROM public.capture_frame_hourly {proto} \
        ...
        .query_one(
            &format!(
                "SELECT (EXTRACT(EPOCH FROM {min_expr}) * 1000000)::float8 AS min_us, \
                 (EXTRACT(EPOCH FROM {max_expr}) * 1000000)::float8 AS max_us \
                 FROM {table} {proto}"
            ),
            &args.refs(),
        )
...
pub async fn payloads(client: &Client, p: &PayloadsParams) -> Result<Vec<Vec<u8>>, String> {
    ...
    let mut args = Args::default();
    let sql = format!(
        "SELECT data_bytes FROM public.capture_frame {} ORDER BY ts DESC LIMIT {limit}",
        p.filter.where_clause(&mut args)
    );
    let rows = client
        .query(&sql, &args.refs())
        .await
