...
impl SqliteCache {
    ...
    pub fn adopt(&mut self, legacy: &Path) -> Result<u64> {
        ...
        let moved = old.rows;
        if moved == 0 {
            return old.delete().map(|()| 0);
        }

        if self.rows == 0 {
            // The upgrade case is an empty destination created moments ago, and
            ...
            // copy. Only this path pays for a second open.
            old = Self::open(legacy, u64::MAX)?;
        }
...
/// them again: a caller that forgets `-wal` reintroduces the size bug where a
/// cache reported far less than it occupied, because the write-ahead log was
/// not counted.
...
mod tests {
    ...
    /// produced by the retired Python server's `DiskCache`. Three frames: a classic one,
