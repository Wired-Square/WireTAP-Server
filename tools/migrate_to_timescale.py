...
on --source-dsn into `public.capture_frame` on --target-dsn, one day at a time:
...
The source reads `public.can_frame`, which is the table on a legacy archive and
a CAN-only view on a migrated one — either works, and it needs only SELECT. The
target is written as `public.capture_frame`, the base table: PostgreSQL cannot
COPY into a view, and `public.can_frame` is one on a current schema.

To reach the container's Postgres, temporarily publish its port (uncomment
"127.0.0.1:5432:5432" in crates/wiretap-backend/docker-compose.yml) or run this
...
def ensure_target(tgt):
    ...
    if query1(tgt, "SELECT to_regclass('public.capture_frame')")[0] is None:
        sys.exit("target has no public.capture_frame table — apply crates/wiretap-backend/schema/init_schema.sql to it first")
...
def migrate_day(src, tgt, day):
    ...
    buf.seek(0)
    with tgt.cursor() as cur:
        cur.execute("DELETE FROM public.capture_frame WHERE protocol = 'can' "
                    "AND ts >= %s::date AND ts < %s::date + INTERVAL '1 day'", (day, day))
        # COLS, not COLS + protocol: the binary stream came from the source with
        # exactly these nine columns, so naming a tenth here would not match it.
        # `protocol` takes its column default, which is 'can' — right for every
        # row a legacy archive can hold.
        cur.copy_expert(f"COPY public.capture_frame ({COLS}) FROM STDIN WITH BINARY", buf)
    ...
    try:
        with tgt.cursor() as cur:
            cur.execute(
                ...
                "FROM show_chunks('public.capture_frame', newer_than => %s::date, "
...
def refresh_rollup(tgt):
    if query1(tgt, "SELECT to_regclass('public.capture_frame_hourly')")[0] is None:
        return
    start, end = query1(tgt, "SELECT date_trunc('month', min(ts)), max(ts) FROM public.capture_frame")
    if start is None:
        return
    print("refreshing capture_frame_hourly...")
    tgt.autocommit = True
    with tgt.cursor() as cur:
        while start < end:
            nxt = min((start + datetime.timedelta(days=32)).replace(day=1), end)
            cur.execute("CALL refresh_continuous_aggregate('public.capture_frame_hourly', %s, %s)",
                        (start, nxt))
            print(f"  {start:%Y-%m}")
            start = nxt
    tgt.autocommit = False
